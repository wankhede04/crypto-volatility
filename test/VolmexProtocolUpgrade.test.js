...
describe("VolmexProtocolUpgradeable", function() {
  it("Can conserve state between upgrades", async () => {
    const PositionTokenContract = await ethers.getContractFactory("VolmexPositionToken");
    ...
    const ethVShortInstance = await PositionTokenContract.deploy("iETHV", "iETHV");
    await ethVShortInstance.deployed();

    const VolmexProtocol = await ethers.getContractFactory("VolmexProtocol");
    const VolmexProtocolUpgrade = await ethers.getContractFactory("VolmexProtocolUpgradeMock");
    ...
    const upgraded = await upgrades.upgradeProxy(instance.address, VolmexProtocolUpgrade);

    expect(originalProxy.minimumCollateralQty).to.be.equal(await upgraded.minimumCollateralQty());
    ...
    expect(originalProxy.issuanceFees).to.be.equal(await upgraded.issuanceFees());

    expect(originalProxy.redeemFees).to.be.equal(await upgraded.redeemFees());

    expect(originalProxy.accumulatedFees).to.be.equal(await upgraded.accumulatedFees());
  });
  ...
  it("should be able to access new function addition", async () => {
    ...
    const transferDevFees = await upgraded.transferDevFees("0xeBac98733b0a25FcDd56Bd078FFCC2350B6ba403");
